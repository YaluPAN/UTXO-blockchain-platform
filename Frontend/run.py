        ...
        while ErrorFlag:
            try:
                ...
                return render_template(
                    "transactions.html", allTransactions=allTxs, refreshtime=10
                )
            except:
                ErrorFlag = True
                return render_template(
                    "transactions.html", allTransactions={}, refreshtime=10
                )
...
def block():
    ...
    if request.args.get("blockHeader"):
        return redirect(
            url_for("showBlock", blockHeader=request.args.get("blockHeader"))
        )
    else:
        blocks = readDatabase()
...
def create_address():
    ...
    acct.createKeys()
    address_info = {
        "publicAddress": acct.PublicAddress,
        "privateKey": str(acct.privateKey),
    }
    return jsonify({"addressInfo": address_info})
